...
function getSharedFigmaPaths(sharedFigmaRoot, parsedFigma) {
  ...
  return {
    ...
    implementationSpec: path.join(cacheDir, 'implementation-spec.json'),
  };
}
...
function primeRunFigmaDirFromShared(sharedPaths, figmaDir) {
  ...
  copyFileIfExists(sharedPaths.implementationSpec, path.join(figmaDir, 'implementation-spec.json'));
}
...
function persistRunFigmaDirToShared(figmaDir, sharedPaths) {
  ...
  copyFileIfExists(path.join(figmaDir, 'implementation-spec.json'), sharedPaths.implementationSpec);
}
...
async function prepareCompareOnlyState(figmaUrl, figmaDir, sharedFigmaRoot) {
  ...
  return {
    parsedFigma,
    fetchedFigma: { ok: true, fileKey: parsedFigma.fileKey, nodeId: parsedFigma.nodeId },
    referenceImagePath,
    exportJson: { ok: true, compareOnly: true },
    exportAttempts: [],
    viewport,
    sharedPaths,
  };
