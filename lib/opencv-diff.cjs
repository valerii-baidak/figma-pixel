...
async function analyzeDiff(referencePath, screenshotPath, diffPath, outputReportPath, figmaNodePath) {
  ...
  try {
    ...
    rawRegions.sort((regionA, regionB) => (regionB.area - regionA.area) || (regionB.meanAbsDiff - regionA.meanAbsDiff));
...
async function analyzeDiffTiles(referencePath, screenshotPath, diffPath, outputReportPath, figmaNodePath, tiles) {
  if (!referencePath || !screenshotPath || !diffPath || !outputReportPath) {
    throw new Error('Missing required arguments for analyzeDiffTiles');
  }
  ...
  for (const tile of tiles) {
    ...
    try {
      ...
      for (let i = 0; i < contours.size(); i += 1) {
        ...
        const area = rect.width * rect.height;
        if (area < Math.max(64, Math.floor(sliceArea * 0.002))) continue;
        ...
        tileRegions.push({
          ...
          area,
          tileY: yStart,
          coveragePercent: +((area / sliceArea) * 100).toFixed(2),
    ...
    tileRegions.sort((regionA, regionB) => (regionB.area - regionA.area) || (regionB.meanAbsDiff - regionA.meanAbsDiff));
  ...
  allRegions.sort((regionA, regionB) => (regionB.area - regionA.area) || (regionB.meanAbsDiff - regionA.meanAbsDiff));
  ...
  const summary = largestRegions.slice(0, 5).map((region) => {
    const layers = region.figmaLayers?.length
      ? ` → ${region.figmaLayers.slice(0, 2).map((l) => l.name).join(', ')}`
      : '';
    return `difference block at ${region.zone} (${region.width}x${region.height} px, ${region.coveragePercent}% of tile)${layers}`;
  });
